type TraitType = 'narrative' | 'mechanical' | 'talents';
...
interface Trait {
    ...
    name: string;
    description: string;
    type: TraitType; // Narrative has no effects, will show only a String. Mechanical have special effect that can affect to combatStats or improve actions that normaly can't do (must have unique and specific functions for each mechanical traits). Talent affects to Talents.
}

interface PositiveTrait extends Trait {
    level1: Effect[];
    level2?: Effect[];
    level3?: Effect[];
}

interface NegativeTrait extends Trait {
    effect: Effect[];
    incompatibility: string[];
};


const PositiveTraitList: PositiveTrait[] = [
    {
        id: "bully",
        name: "Abusón/a",
...
const NegativeTraitList: NegativeTrait[] = [
