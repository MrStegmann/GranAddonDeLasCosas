...
function GAC:CreateReadOnlyCard(parent, group, statLabelsTable)
    ...
    local attRow, nameLabel = GAC:CreateStatLabel(headerBg, GAC:_(group.name), group.name, false, statLabelsTable)
    ...
    for _, talent in ipairs(group.talents) do
        local talRow = GAC:CreateStatLabel(card, GAC:_(talent), talent, true, statLabelsTable)
