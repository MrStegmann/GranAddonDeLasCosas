...
function GAC:UpdatePlayerPlate()
    ...
    GAC:SafeCall(function()
        ...
        -- 3. Modificar el nivel, dragón y su color según categoría
        ...
        local levelText = PlayerFrameTextureFramePlayerLevelText or PlayerLevelText
        if levelText then
        ...
        if not parentForOverlay then return end
        ...
        if category == "noob" then
            ...
            elseif category == "elite" or category == "élite" then
                overlay:SetTexture("Interface\\TargetingFrame\\UI-TargetingFrame-Elite")
                overlay:SetTexCoord(1, 0, 0, 1) -- Voltear horizontalmente
            elseif category == "boss" or category == "jefe" then
                overlay:SetTexture("Interface\\TargetingFrame\\UI-TargetingFrame-Elite")
                overlay:SetTexCoord(1, 0, 0, 1) -- Voltear horizontalmente
...
function GAC:InitializePlayerPlate()
    ...
    -- 6. Listener de eventos de cliente para actualizar PlayerPlate al entrar al mundo o cambiar salud
    ...
    if GAC.UpdatePlayerPlate then GAC:UpdatePlayerPlate() end
end
